...
os.environ['MODELSCOPE_CACHE'] = '/cache/Q3o/modelscope'
...
def main():
    parser = argparse.ArgumentParser(
        description='Classify child actions and functions in videos using fine-tuned Qwen3 Omni LoRA.'
    )
    parser.add_argument(
        '--input', '-i',
        default='Data/',
    ...
    # --- Prompt Definition (Joint Action + Function Classification) ---
    question = """Your task is to classify the primary communicative action and function of the child in this video. The child is non- or minimally-speaking and has profound autism and other complex neurodevelopmental disorders. Choose the most accurate option below.
    ...
    with open(csv_path, 'a', newline='', encoding='utf-8') as csvfile:
        ...
        for i, video_path in enumerate(files_to_process, 1):
            ...
            try:
                ...
                # Try to parse "Action: X" and "Function: X" patterns
                action_match = re.search(r'Action:\s*(\d)', clean_response, re.IGNORECASE)
                function_match = re.search(r'Function:\s*(\d)', clean_response, re.IGNORECASE)
                ...
                if action_class == "N/A" or function_class == "N/A":
                    digits = re.findall(r'\b([0-5])\b', clean_response[-100:])
                    if len(digits) >= 2:
                        if action_class == "N/A":
